...
`assets/art/` 中的 70 张 PNG 共约 **1.09 MiB**，单张最大约 **190 KiB**；运行时加载 65 个纹理 key（60 个 PNG 文件）。`assets/source/` 另保留墙面／障碍和三色鹦鹉的压缩源图集，不参与游戏加载。无需生成工具即可运行。
...
- `src/gfx/TextureFactory.js`：仅继续生成绿宝石、生命水晶；所有玩家与场景的旧工厂调用已停用。

...
`assets/art/` 中的 64 张 PNG 共约 **1.08 MiB**，单张最大约 **190 KiB**；运行时加载 59 个纹理 key（54 个 PNG 文件）。`assets/source/` 另保留两张压缩后的修订版源图集，不参与游戏加载。无需生成工具即可运行。
...
PNG 透明度已量化为清晰边缘，游戏开启 `pixelArt` 和 `roundPixels`。碰撞尺寸取实体 `w/h`，不从 PNG 的透明轮廓推导碰撞。蜜蜂和发光鱿鱼已按反馈放大两倍，显示／碰撞尺寸分别为 **70 × 50** 和 **68 × 60**；其余实体碰撞尺寸未改。

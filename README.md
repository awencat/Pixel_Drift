...
- 潜影贝：在末地和花开尖塔随墙生成（每面墙概率分别为 55%、40%），支持上下左右贴附，随墙移动、随墙回收。完整穿屏发射 3–5 颗定向米色粘液弹，命中扣血并禁冲刺 3 秒；再次有效命中刷新为 3 秒，不累加。冲刺可击碎粘液弹，受伤无敌期间不会再次施加减益。
...
暂用现有贴图染色并缩放表现，未生成新 PNG。新实体代码在 `src/entities/NewEnemyEntities.js`；群系槽位和贴墙概率在 `src/config/biomes.js`。尚未填写障碍配置的群系默认使用墙／浮岛／脆弱障碍权重 4／3／3。
